...
int pmp_write_csr_addr(u16 address, struct csr_mapping *map, uxlen val)
{
	...
	if (pmpaddr_i < (PMP_NR_ADDR_REGS - 1)) {
		...
		if ((addr_mode_next_entry == pmp_a_tor)
		    && CHECK_BIT(*next_cfg_ptr, PMP_CFG_L_BIT)) {
			return 0;
		}
	}
...
int
pmp_mem_check(struct hart *hart, privilege_level curr_priv, uxlen addr,
	...
	for (i = 0; i < PMP_NR_CFG_REGS; i++) {
		...
		for (j = 0; j < sizeof(uxlen); j++) {
			...
			if ((curr_priv == machine_mode)
			    && !CHECK_BIT(cfg_ptr[j], PMP_CFG_L_BIT))
				continue;
			...
			case pmp_a_tor:
				if (addr_count == 0) {
					...
					pmp_addr_size =
					    (hart->csr_store.
					     pmpaddr[addr_count] << 2);
				} else {
					pmp_addr_start =
					    (hart->csr_store.
					     pmpaddr[addr_count - 1] << 2);
					pmp_addr_size =
					    (hart->csr_store.
					     pmpaddr[addr_count] << 2)
					    - pmp_addr_start;
			...
			case pmp_a_napot:
				...
				if (hart->csr_store.pmpaddr[addr_count] ==
					...
					pmp_addr_size =
					    get_pmp_napot_size_from_pmpaddr
					    (hart->csr_store.
					     pmpaddr[addr_count]);
					pmp_addr_start =
					    get_pmp_napot_addr_from_pmpaddr
					    (hart->csr_store.
					     pmpaddr[addr_count]);
				}
				break;
			 ...
			 */
			if (upper_addr_match && lower_addr_match)
				return (curr_access_flags & allowed_access) ?
				    0 : -1;
		}
	}
...
int
pmp_check(struct hart *hart, privilege_level priv_level,

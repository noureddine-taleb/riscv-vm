...
int
mmu_virt_to_phys(struct hart *hart,
		 ...
		 uxlen *virt_addr, bus_access_type access_type, u8 mxr, u8 sum)
{
	...
	for (i = (SV39_LEVELS - 1), j = 0; i >= 0; i--, j++) {
		 ...
		 */
		if ((!(pte_flags & MMU_PAGE_VALID))
		    || ((!(pte_flags & MMU_PAGE_READ))
			&& (pte_flags & MMU_PAGE_WRITE))
		    || (pte_flags & MMU_PAGE_PBMT)
		    || (pte_flags & MMU_PAGE_N)
		    || (pte_flags & MMU_PAGE_RESERVED)
		    ) {
			MMU_DEBUG
			    ("page fault: pte.v = 0, or if pte.r = 0 and pte.w = 1 access_type: %d a: "
			     PRINTF_FMT " virt_addr: " PRINTF_FMT " pte: "
			     PRINTF_FMT " pte_addr: " PRINTF_FMT
			     " flags: %x curr_priv: %d level: %d pc: "
			     PRINTF_FMT " value: " PRINTF_FMT "\n", access_type,
	...
	if ((access_type == bus_read_access)
	    && (ACCESS_TYPE_TO_MMU(bus_instr_access) & pte_flags) && mxr)
		pte_flags |= MMU_PAGE_READ;
	...
	u64 phys_addr_translation[SV39_LEVELS] = {
		(ppn[2] << 30) | (ppn[1] << 21) | (ppn[0] << 12) | (*virt_addr &
								    0xfff),
	 ...
	 */
	if ((!(pte_flags & MMU_PAGE_ACCESSED))
	    || ((access_type == bus_write_access)
		&& !(pte_flags & MMU_PAGE_DIRTY))) {
...
int
vm_check(struct hart *hart, privilege_level priv_level,
	 ...
	 __unused void *value, __unused u8 len)
{
	...
	u8 mxr = CHECK_BIT(hart->csr_store.status,
			   TRAP_XSTATUS_MXR_BIT) ? 1 : 0;
	u8 sum = CHECK_BIT(hart->csr_store.status,
			   TRAP_XSTATUS_SUM_BIT) ? 1 : 0;
